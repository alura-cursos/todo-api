package br.com.alura.todoapi.log
...
class ApplicationLogger(val logger: Logger = LoggerFactory.getLogger(ApplicationLogger::class.java)) {
    ...
    @Pointcut("execution(* br.com.alura.todoapi..service..*(..))")
    fun services() {
    }
    @Pointcut("execution(* br.com.alura.todoapi..controller..*(..))")
    fun controllers() {
    }
    ...
    fun profile(joinPoint: ProceedingJoinPoint): Any {
        ...
        return try {
            ...
            val log = buildMessageLog(targetClass,
                    targetMethod, targetParameters)
            logger.info(log)
        }
    }

    private fun buildMessageLog(className: String?,
                                methodName: String?,
                                parameters: Array<Any>): String {
